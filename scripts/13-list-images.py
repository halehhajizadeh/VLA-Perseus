    ...
    for s in spw:
        for channel in channels:
            if s in existing_spw:
                files_list.append(path+"Images/img"+str(nit)+"/fits/546-spw"+str(s)+'-'+ str(channel)+"-2.5arcsec-nit"+str(nit)+"-"+str(thresh)+"-"+str(stok)+'.image.smo.fits')
            elif s in non_spw:
                files_list.append(path+"Images/img"+str(nit)+"/fits/empty_channel.fits")    
        
    np.savetxt(path+'Images/img'+str(nit)+'/546_stokes' + str(stok) +'.txt', files_list, fmt ='%s')            
